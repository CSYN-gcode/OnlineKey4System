function GetCurrentFYInkDataFAC() {
    $.ajax({
        url: 'get_current_ink_data_fac',
        method: 'get',
        dataType: 'json',
        data: {
            fiscal_year: $('#selFiscalYearInkFAC').val()
        },
        success: function (response) {
                    ...
                    var aprilTarget = 0;
                     if(data[datas.findIndex(checkMonth)].target != null) {
                        // console.log('Third IF april');
                         $('.april-ink-fac-current-fy-target').html(data[datas.findIndex(checkMonth)].target);

                         aprilTarget = data[datas.findIndex(checkMonth)].target;
                     } else {
                        // console.log('Third ELSE april');
                        $('.april-ink-fac-current-fy-target').html('-');
                     }
                     ...
                     if(data[datas.findIndex(checkMonth)].actual != null) {
                        ...
                        $('.april-ink-fac-actual-target').html(aprilDifference);
                        
                        $('.april-ink-fac-tricolor').html(aprilCummulative);
                        ...
                        if(data[datas.findIndex(checkMonth)].target > data[datas.findIndex(checkMonth)].actual) {
                         ...
                         $('.april-ink-fac-current-fy-actual').html(data[datas.findIndex(checkMonth)].actual + '&nbsp;&nbsp;&nbsp;&nbsp;' + icon);
                     } else {
                        // console.log('Fourth ELSE april');
                        $('.april-ink-fac-tricolor').html('-');
                        $('.april-ink-fac-current-fy-actual').html('-');
                     }
                } else {
                    // console.log('Second ELSE april');
                    // $('.april-ink-fac-actual-target').html('-');
                    $('.april-ink-fac-current-fy-target').html('-');
                    $('.april-ink-fac-current-fy-actual').html('-');
                }
            } else {
            ...
            if(jQuery.inArray(5, datas) != -1) {
                    ...
                    var mayTarget = 0;
                     if(data[datas.findIndex(checkMonth)].target != null) {
                        ...
                        mayTarget = data[datas.findIndex(checkMonth)].target;
                         ...
                         $('.may-ink-fac-current-fy-target').html(data[datas.findIndex(checkMonth)].target);
                     } else {
                        // console.log('Third ELSE may');
                        $('.may-ink-fac-current-fy-target').html('-');
                     }
                     ...
                     if(data[datas.findIndex(checkMonth)].actual != null) {
                        ...
                        $('.may-ink-fac-actual-target').html(mayDifference);
                        $('.may-ink-fac-tricolor').html(mayCummulative);
                        ...
                        if(data[datas.findIndex(checkMonth)].target > data[datas.findIndex(checkMonth)].actual) {
                         ...
                         $('.may-ink-fac-current-fy-actual').html(data[datas.findIndex(checkMonth)].actual + '&nbsp;&nbsp;&nbsp;&nbsp;' + icon);
                     } else {
                        // console.log('Fourth ELSE may');
                        $('.may-ink-fac-tricolor').html('-');
                        $('.may-ink-fac-current-fy-actual').html('-');
                     }
                ...
                $('.may-ink-fac-actual-target').html('-');
                $('.may-ink-fac-tricolor').html('-');
                $('.may-ink-fac-current-fy-target').html('-');
                $('.may-ink-fac-current-fy-actual').html('-');
            }
            ...
            if(jQuery.inArray(6, datas) != -1) {
                    ...
                    var juneTarget = 0;
                     if(data[datas.findIndex(checkMonth)].target != null) {
                        ...
                        juneTarget = data[datas.findIndex(checkMonth)].target;
                         ...
                         $('.june-ink-fac-current-fy-target').html(data[datas.findIndex(checkMonth)].target);
                     } else {
                        // console.log('Third ELSE june');
                        $('.june-ink-fac-current-fy-target').html('-');
                     }
                     ...
                     if(data[datas.findIndex(checkMonth)].actual != null) {
                        ...
                        $('.june-ink-fac-actual-target').html(juneDifference);
                        $('.june-ink-fac-tricolor').html(juneCummulative);
                        ...
                        if(data[datas.findIndex(checkMonth)].target > data[datas.findIndex(checkMonth)].actual) {
                         ...
                         $('.june-ink-fac-current-fy-actual').html(data[datas.findIndex(checkMonth)].actual + '&nbsp;&nbsp;&nbsp;&nbsp;' + icon);
                     } else {
                        // console.log('Fourth ELSE june');
                        $('.june-ink-fac-tricolor').html('-');
                        $('.june-ink-fac-current-fy-actual').html('-');
                     }
                ...
                $('.june-ink-fac-actual-target').html('-');
                $('.june-ink-fac-tricolor').html('-');
                $('.june-ink-fac-current-fy-target').html('-');
                $('.june-ink-fac-current-fy-actual').html('-');
            }
            ...
            if(jQuery.inArray(7, datas) != -1) {
                    ...
                    var julyTarget = 0;
                     if(data[datas.findIndex(checkMonth)].target != null) {
                        ...
                        julyTarget = data[datas.findIndex(checkMonth)].target;
                         ...
                         $('.july-ink-fac-current-fy-target').html(data[datas.findIndex(checkMonth)].target);
                     } else {
                        // console.log('Third ELSE july');
                        $('.july-ink-fac-current-fy-target').html('-');
                     }
                     ...
                     if(data[datas.findIndex(checkMonth)].actual != null) {
                        ...
                        $('.july-ink-fac-actual-target').html(julyDifference);
                        $('.july-ink-fac-tricolor').html(julyCummulative);
                        ...
                        if(data[datas.findIndex(checkMonth)].target > data[datas.findIndex(checkMonth)].actual) {
                         ...
                         $('.july-ink-fac-current-fy-actual').html(data[datas.findIndex(checkMonth)].actual + '&nbsp;&nbsp;&nbsp;&nbsp;' + icon);
                     } else {
                        // console.log('Fourth ELSE july');
                        $('.july-ink-fac-tricolor').html('-');
                        $('.july-ink-fac-current-fy-actual').html('-');
                     }
                ...
                $('.july-ink-fac-actual-target').html('-');
                $('.july-ink-fac-tricolor').html('-');
                $('.july-ink-fac-current-fy-target').html('-');
                $('.july-ink-fac-current-fy-actual').html('-');
            }
            ...
            if(jQuery.inArray(8, datas) != -1) {
                    ...
                    var augustTarget = 0;
                     if(data[datas.findIndex(checkMonth)].target != null) {
                        ...
                        augustTarget = data[datas.findIndex(checkMonth)].target
                         ...
                         $('.august-ink-fac-current-fy-target').html(data[datas.findIndex(checkMonth)].target);
                     } else {
                        // console.log('Third ELSE august');
                        $('.august-ink-fac-current-fy-target').html('-');
                     }
                     ...
                     if(data[datas.findIndex(checkMonth)].actual != null) {
                        ...
                        $('.august-ink-fac-actual-target').html(augustDifference);
                        $('.august-ink-fac-tricolor').html(augustCummulative);
                        ...
                        if(data[datas.findIndex(checkMonth)].target > data[datas.findIndex(checkMonth)].actual) {
                         ...
                         $('.august-ink-fac-current-fy-actual').html(data[datas.findIndex(checkMonth)].actual + '&nbsp;&nbsp;&nbsp;&nbsp;' + icon);
                     } else {
                        // console.log('Fourth ELSE august');
                        $('.august-ink-fac-tricolor').html('-');
                        $('.august-ink-fac-current-fy-actual').html('-');
                     }
                ...
                $('.august-ink-fac-actual-target').html('-');
                $('.august-ink-fac-tricolor').html('-');
                $('.august-ink-fac-current-fy-target').html('-');
                $('.august-ink-fac-current-fy-actual').html('-');
            }
            ...
            if(jQuery.inArray(9, datas) != -1) {
                    ...
                    var septemberTarget = 0;
                     if(data[datas.findIndex(checkMonth)].target != null) {
                        ...
                        septemberTarget = data[datas.findIndex(checkMonth)].target;
                         ...
                         $('.september-ink-fac-current-fy-target').html(data[datas.findIndex(checkMonth)].target);
                     } else {
                        // console.log('Third ELSE september');
                        $('.september-ink-fac-current-fy-target').html('-');
                     }
                     ...
                     if(data[datas.findIndex(checkMonth)].actual != null) {
                        ...
                        $('.september-ink-fac-actual-target').html(septemberDifference);
                        $('.september-ink-fac-tricolor').html(septemberCummulative);
                        ...
                        if(data[datas.findIndex(checkMonth)].target > data[datas.findIndex(checkMonth)].actual) {
                         ...
                         $('.september-ink-fac-current-fy-actual').html(data[datas.findIndex(checkMonth)].actual + '&nbsp;&nbsp;&nbsp;&nbsp;' + icon);
                     } else {
                        // console.log('Fourth ELSE september');
                        $('.september-ink-fac-tricolor').html('-');
                        $('.september-ink-fac-current-fy-actual').html('-');
                     }
                ...
                $('.september-ink-fac-actual-target').html('-');
                $('.september-ink-fac-tricolor').html('-');
                $('.september-ink-fac-current-fy-target').html('-');
                $('.september-ink-fac-current-fy-actual').html('-');
            }
            ...
            if(jQuery.inArray(10, datas) != -1) {
                    ...
                    var octoberTarget = 0;
                     if(data[datas.findIndex(checkMonth)].target != null) {
                        ...
                        octoberTarget = data[datas.findIndex(checkMonth)].target;
                         ...
                         $('.october-ink-fac-current-fy-target').html(data[datas.findIndex(checkMonth)].target);
                     } else {
                        // console.log('Third ELSE october');
                        $('.october-ink-fac-current-fy-target').html('-');
                     }
                     ...
                     if(data[datas.findIndex(checkMonth)].actual != null) {
                        ...
                        $('.october-ink-fac-actual-target').html(octoberDifference);
                        $('.october-ink-fac-tricolor').html(octoberCummulative);
                        ...
                        if(data[datas.findIndex(checkMonth)].target > data[datas.findIndex(checkMonth)].actual) {
                         ...
                         $('.october-ink-fac-current-fy-actual').html(data[datas.findIndex(checkMonth)].actual + '&nbsp;&nbsp;&nbsp;&nbsp;' + icon);
                     } else {
                        // console.log('Fourth ELSE october');
                        $('.october-ink-fac-tricolor').html('-');
                        $('.october-ink-fac-current-fy-actual').html('-');
                     }
                ...
                $('.october-ink-fac-actual-target').html('-');
                $('.october-ink-fac-tricolor').html('-');
                $('.october-ink-fac-current-fy-target').html('-');
                $('.october-ink-fac-current-fy-actual').html('-');
            }
            ...
            if(jQuery.inArray(11, datas) != -1) {
                    ...
                    var novemberTarget = 0;
                     if(data[datas.findIndex(checkMonth)].target != null) {
                        ...
                        novemberTarget = data[datas.findIndex(checkMonth)].target;
                         ...
                         $('.november-ink-fac-current-fy-target').html(data[datas.findIndex(checkMonth)].target);
                     } else {
                        // console.log('Third ELSE november');
                        $('.november-ink-fac-current-fy-target').html('-');
                     }
                     ...
                     if(data[datas.findIndex(checkMonth)].actual != null) {
                        ...
                        $('.november-ink-fac-actual-target').html(novemberDifference);
                        $('.november-ink-fac-tricolor').html(novemberCummulative);
                        ...
                        if(data[datas.findIndex(checkMonth)].target > data[datas.findIndex(checkMonth)].actual) {
                         ...
                         $('.november-ink-fac-current-fy-actual').html(data[datas.findIndex(checkMonth)].actual + '&nbsp;&nbsp;&nbsp;&nbsp;' + icon);
                     } else {
                        // console.log('Fourth ELSE november');
                        $('.november-ink-fac-tricolor').html('-');
                        $('.november-ink-fac-current-fy-actual').html('-');
                     }
                ...
                $('.november-ink-fac-actual-target').html('-');
                $('.november-ink-fac-tricolor').html('-');
                $('.november-ink-fac-current-fy-target').html('-');
                $('.november-ink-fac-current-fy-actual').html('-');
            }
            ...
            if(jQuery.inArray(12, datas) != -1) {
                    ...
                    var decemberTarget = 0;
                     if(data[datas.findIndex(checkMonth)].target != null) {
                        ...
                        decemberTarget = data[datas.findIndex(checkMonth)].target;
                         ...
                         $('.december-ink-fac-current-fy-target').html(data[datas.findIndex(checkMonth)].target);
                     } else {
                        // console.log('Third ELSE december');
                        $('.december-ink-fac-current-fy-target').html('-');
                     }
                     ...
                     if(data[datas.findIndex(checkMonth)].actual != null) {
                        ...
                        $('.december-ink-fac-actual-target').html(decemberDifference);
                        $('.december-ink-fac-tricolor').html(decemberCummulative);
                        ...
                        if(data[datas.findIndex(checkMonth)].target > data[datas.findIndex(checkMonth)].actual) {
                         ...
                         $('.december-ink-fac-current-fy-actual').html(data[datas.findIndex(checkMonth)].actual + '&nbsp;&nbsp;&nbsp;&nbsp;' + icon);
                     } else {
                        // console.log('Fourth ELSE december');
                        $('.december-ink-fac-tricolor').html('-');
                        $('.december-ink-fac-current-fy-actual').html('-');
                     }
                ...
                $('.december-ink-fac-actual-target').html('-');
                $('.december-ink-fac-tricolor').html('-');
                $('.december-ink-fac-current-fy-target').html('-');
                $('.december-ink-fac-current-fy-actual').html('-');
            }
            ...
            if(jQuery.inArray(1, datas) != -1) {
                 ...
                 if(datas.findIndex(checkMonth) != -1) {
                    ...
                    var januaryTarget = 0;
                     if(data[datas.findIndex(checkMonth)].target != null) {
                        ...
                        $('.january-ink-fac-current-fy-target').html(data[datas.findIndex(checkMonth)].target);

                            
                     } else {
                        // console.log('Third ELSE January');
                        $('.january-ink-fac-current-fy-target').html('-');
                     }
                    ...
                    var januaryActual = 0;
                     ...
                     if(data[datas.findIndex(checkMonth)].actual != null) {
                        ...
                        $('.january-ink-fac-actual-target').html(januaryDifference);
                        ...
                        if(checkCummulative != 'NaN') {
                            // console.log('asd');
                            $('.january-ink-fac-tricolor').html(januaryCummulative);
                        } else {
                            // console.log('asda');
                            $('.january-ink-fac-tricolor').html('-');
                        }
                        ...
                        if(data[datas.findIndex(checkMonth)].target > data[datas.findIndex(checkMonth)].actual) {
                         ...
                         $('.january-ink-fac-current-fy-actual').html(data[datas.findIndex(checkMonth)].actual + '&nbsp;&nbsp;&nbsp;&nbsp;' + icon);
                     } else {
                        $('.january-ink-fac-tricolor').html('-');
                        $('.january-ink-fac-current-fy-actual').html('-');
                     }
                ...
                $('.january-ink-fac-actual-target').html('-');
                $('.january-ink-fac-tricolor').html('-');
                $('.january-ink-fac-current-fy-target').html('-');
                $('.january-ink-fac-current-fy-actual').html('-');
            }
            ...
            if(jQuery.inArray(2, datas) != -1) {
                    ...
                    var februaryTarget = 0;
                     if(data[datas.findIndex(checkMonth)].target != null) {
                        ...
                        februaryTarget = data[datas.findIndex(checkMonth)].target;
                         ...
                         $('.february-ink-fac-current-fy-target').html(data[datas.findIndex(checkMonth)].target);
                     } else {
                        // console.log('Third ELSE february');
                        $('.february-ink-fac-current-fy-target').html('-');
                     ...
                     if(data[datas.findIndex(checkMonth)].actual != null) {
                        ...
                        $('.february-ink-fac-actual-target').html(februaryDifference);

                        if(checkCummulative != 'NaN') {
                            // console.log('asd');
                            $('.february-ink-fac-tricolor').html(februaryCummulative);
                        } else {
                            // console.log('asda');
                            $('.february-ink-fac-tricolor').html('-');
                        }
                        ...
                        if(data[datas.findIndex(checkMonth)].target > data[datas.findIndex(checkMonth)].actual) {
                         ...
                         $('.february-ink-fac-current-fy-actual').html(data[datas.findIndex(checkMonth)].actual + '&nbsp;&nbsp;&nbsp;&nbsp;' + icon);
                     } else {
                        // console.log('Fourth ELSE february');
                        $('.february-ink-fac-tricolor').html('-');
                        $('.february-ink-fac-current-fy-actual').html('-');
                     }
                ...
                $('.february-ink-fac-actual-target').html('-');
                $('.february-ink-fac-tricolor').html('-');
                $('.february-ink-fac-current-fy-target').html('-');
                $('.february-ink-fac-current-fy-actual').html('-');
            }
            ...
            if(jQuery.inArray(3, datas) != -1) {
                    ...
                    var marchTarget = 0;
                     if(data[datas.findIndex(checkMonth)].target != null) {
                        ...
                        marchTarget = data[datas.findIndex(checkMonth)].target;
                         ...
                         $('.march-ink-fac-current-fy-target').html(data[datas.findIndex(checkMonth)].target);
                     } else {
                        // console.log('Third ELSE march');
                        $('.march-ink-fac-current-fy-target').html('-');
                     }
                     ...
                     if(data[datas.findIndex(checkMonth)].actual != null) {
                        ...
                        $('.march-ink-fac-actual-target').html(marchDifference); 

                        if(checkCummulative != 'NaN') {
                            // console.log('asd');
                            $('.march-ink-fac-tricolor').html(marchCummulative);
                        } else {
                            // console.log('asda');
                            $('.march-ink-fac-tricolor').html('-');
                        }
                        ...
                        if(data[datas.findIndex(checkMonth)].target > data[datas.findIndex(checkMonth)].actual) {
                         ...
                         $('.march-ink-fac-current-fy-actual').html(data[datas.findIndex(checkMonth)].actual + '&nbsp;&nbsp;&nbsp;&nbsp;' + icon);
                     } else {
                        // console.log('Fourth ELSE march');
                        $('.march-ink-fac-tricolor').html('-');
                        $('.march-ink-fac-current-fy-actual').html('-');
                     }
                ...
                $('.march-ink-fac-actual-target').html('-');
                $('.march-ink-fac-tricolor').html('-');
                $('.march-ink-fac-current-fy-target').html('-');
                $('.march-ink-fac-current-fy-actual').html('-');
            }
            ...
            if(sumTarget != 0 && sumTarget != null) {
                ...
                $('.total-ink-fac-current-fy-target').html(sumTarget); 
            } else {
                $('.total-ink-fac-current-fy-target').html('-'); 
            }
            ...
            if(sumActual != 0 && sumActual != null) {
                ...
                $('.total-ink-fac-current-fy-actual').html(sumActual); 
            } else {
                $('.total-ink-fac-current-fy-actual').html('-'); 
            }
           ...
           if(difference != 0) {
            $('.total-ink-fac-actual-target').html(difference);
            $('.total-ink-fac-tricolor').html(difference);
        } else {
            $('.total-ink-fac-actual-target').html('-');
            $('.total-ink-fac-tricolor').html('-');
            
        }

            $('.april-ink-fac-current-fy').html('April ' + currentYear);
            $('.may-ink-fac-current-fy').html('May ' + currentYear);
            $('.june-ink-fac-current-fy').html('June ' + currentYear);
            $('.july-ink-fac-current-fy').html('July ' + currentYear);
            $('.august-ink-fac-current-fy').html('August ' + currentYear);
            $('.september-ink-fac-current-fy').html('September ' + currentYear);
            $('.october-ink-fac-current-fy').html('October ' + currentYear);
            $('.november-ink-fac-current-fy').html('November ' + currentYear);
            $('.december-ink-fac-current-fy').html('December ' + currentYear);
            $('.january-ink-fac-current-fy').html('January ' + nextYear);
            $('.february-ink-fac-current-fy').html('February ' + nextYear);
            $('.march-ink-fac-current-fy').html('March ' + nextYear); 
        }
    });
}